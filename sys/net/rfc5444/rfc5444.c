...
void gnrc_rc5444_del_writer_target(const ipv6_addr_t *dst, uint16_t iface)
{
    ...
    for (unsigned i = 0; i < ARRAY_SIZE(_target); i++) {
        _gnrc_rfc5444_target_t *tmp = &_target[i];
        if (tmp->iface == iface && _addr_equals(dst, tmp) && !tmp->empty) {
                         ...
                         size_t length)
{
    ...
    DEBUG("gnrc_rfc5444: sending packet (dst = %s, iface = %d)\n",
          ...
          target->iface);
    DEBUG("  length = %d, buffer = %p", length, (void *)buffer);
